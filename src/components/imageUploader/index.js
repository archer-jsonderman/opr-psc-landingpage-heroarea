...
//Need to handle new file upload->asset, and save Asset content type to page object
export default class Uploader extends React.Component {
  ...
  onClickLinkExisting = async () => {
    ...
    //save selected asset to state
    this.setState({value:selectedAsset})
	//console.log(this.state, ' linkexisting')
    try {
  ...
  */
  ...
  setFieldLink(assetId) {
	  //console.log(this.state, ' initFieldLink')
    return this.props.sdk.space
          .getAsset(this.state.value.sys.id)
          .then(asset => {
	          this.setState({ asset },this.passStateAsProps)
	          })
  }
  ...
  passStateAsProps = () => {
	  this.props.onStateChange('image',this.state.value)
  }
...
//<---uploading new files is not saving them on complete
